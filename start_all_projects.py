"""
start_all_projects.py  —  Multi-Project Launcher for n8n Telegram Bots
======================================================================
Usage:  python start_all_projects.py

Starts one n8n instance on port 5678 with one Cloudflare Tunnel, then
refreshes webhooks for all active projects. Kill with Ctrl+C to clean up.

To add/remove projects, edit the ACTIVE_PROJECTS list below.
"""
...
# ╔═════════════════════════════════════════════════════════════╗
# ║   ADD / REMOVE PROJECTS HERE — match n8n workflow names    ║
# ╚═════════════════════════════════════════════════════════════╝
ACTIVE_PROJECTS = [
    "QR Code Generator Bot",
    "IPStack Telegram Intelligence Bot",
]
...
def refresh_webhooks():
    ...
    for wf in wfs:
        ...
        wname = wf.get("name", "")
        is_target = any(
            proj.lower() in wname.lower() or wname.lower() in proj.lower()
        ...
        try:
            ...
            ok(f"Webhook refreshed: {wname}")
        except Exception as e:
            warn(f"Failed for '{wname}': {e}")

    if matched == 0:
        warn("No matching active workflows found.")
    else:
        ok(f"Refreshed {matched} workflow(s)")

def main():
    sep(f"n8n Multi-Project Launcher — Port {PORT}")
    ...
    print(f"  Projects:    {', '.join(ACTIVE_PROJECTS)}")
